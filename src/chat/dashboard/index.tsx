...
import DashboardChatProvider, { useDashboardChatContext } from './dashboardChatContext';
...
(() => {
    const TGram = forwardRef<any, ITgramOptions>((props, ref) => {
        ...
        const feedback = useCallback(async (payload: IFeedbackPayload) => {
            try {
                const response = await dashboardRepository.feedback(payload, token);
                // TODO: 일단은...
                // @ts-ignore
                if (response.tokenExpired) {
                    props.onTokenExpired();
        ...
        const initialize = useCallback((
            ...
            (async () => {
                ...
                const ragInitData = await dashboardRepository.ragInit(token);
                // TODO: 일단은...
                // @ts-ignore
        ...
        ReactDOM.render(
            <SessionProvider initialSessionId={options.sessionId}>
                <TokenProvider initialToken={options.token}>
                    <DashboardChatProvider repository={dashboardRepository}>
                        <KeyProvider>
